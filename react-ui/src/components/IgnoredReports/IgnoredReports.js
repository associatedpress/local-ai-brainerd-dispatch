...
import "./IgnoredReports.css";
import Banner from '../Banner/Banner';

function IgnoredReports(props) {
  ...
  {
    fetch(baseuri + `/getignoredreports`, {
      headers: {
      ...
      .catch(error => console.error('Error fetching ignored reports:', error));
  } 
  ...
  const handleRestore = () => {
    ...
    fetch(baseuri + `/restorecases`, {
      ...
      .then(response => {
        console.log('Added items to restore list : ', response.json());
        if (response.ok) {
          setMessage("Selected items are added to restore list");
          setIsVisible(true);
          ...
          setMessage("Unable to add items to restore list");
  ...
  return (
    <div className="main-container">
      ...
      <div className="main-box1">
          <h4 className="blotterItems">Ignored Items</h4>
        ...
        <div className="main-box3" >   
          <button className="restorebutton" onClick={handleRestore}>Restore</button>    
